...
class PokeApiTest {
    ...
    @Test
    fun `getAbilityBattleArmor() should return normal data`() {
        ...
        val client = HystrixFeign.builder()
            ...
            .target(PokeApi::class.java, "https://pokeapi.co", FallbackPokeApi())

        val actual = client.getAbilityBattleArmor() as Map<*, *>

        assertTrue(actual.keys.contains("effect_changes"))
    }

    @Test
    fun `getAbilityBattleArmor() with timeout`() {
        // given
        MockServerClient("127.0.0.1", 18080)
            .`when`(
                ...
                HttpRequest.request()
                    .withMethod("GET")
                    .withPath("/api/v2/ability/battle-armor")
            )
            .respond(
        ...
        val actual = client.getAbilityBattleArmor() as Map<*, *>
        assertTrue(actual.keys.contains("effect_changes"))
        assertEquals("error", actual.get("effect_changes"))
    }
}